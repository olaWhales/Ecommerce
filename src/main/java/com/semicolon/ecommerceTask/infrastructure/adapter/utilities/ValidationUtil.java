...
public class ValidationUtil {
    ...
    public static void validateEmail(String email) {
        if (email == null || email.trim().isEmpty() || !email.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$")) {throw new NameNotFoundException(MessageUtil.INVALID_EMAIL);}
    }

    public static void validatePassword(String password) {
        if (password == null || !password.matches("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*?])(?=\\S+$).{8,}$")) {throw new NameNotFoundException(MessageUtil.INVALID_PASSWORD);}
    }
